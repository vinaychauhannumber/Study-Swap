...
export default function Chat() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 h-[calc(100vh-140px)] py-2 items-stretch">
      ...
      <div className="hidden lg:block glass p-5 rounded-3xl border border-slate-800 space-y-6 flex flex-col justify-between">
        <div className="space-y-6">
          <Link to={`/tasks/${taskId}`} className="inline-flex items-center space-x-1 text-xs text-slate-400 hover:text-white transition">
            <ArrowLeft size={13} />
          ...
          {task && (
            <div className="space-y-4">
              <span className="text-[9px] font-semibold bg-slate-900 border border-slate-800 text-indigo-400 px-2 py-0.5 rounded uppercase">
                {task.category}
              </span>
              <h3 className="text-sm font-bold text-slate-100 line-clamp-2 leading-relaxed">{task.title}</h3>
              <p className="text-[11px] text-slate-400 line-clamp-4 leading-relaxed">{task.description}</p>
              
              <div className="space-y-2.5 pt-4 border-t border-slate-900 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-500">Escrow budget:</span>
                  <strong className="text-emerald-400">₹{task.budget}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Target timeline:</span>
                  <strong className="text-slate-300">{task.deadline}</strong>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-3.5 rounded-2xl bg-indigo-950/20 border border-indigo-900/40 text-[10px] text-indigo-300 leading-normal flex items-start space-x-1.5 font-medium">
      ...
      <div className="lg:col-span-3 glass rounded-3xl border border-slate-800 flex flex-col justify-between overflow-hidden">
        
        {/* Chat header */}
        <div className="p-4 bg-slate-950/40 border-b border-slate-900 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Link to="/dashboard" className="p-1.5 rounded-lg hover:bg-slate-900 border border-transparent hover:border-slate-800 text-slate-400 lg:hidden shrink-0">
              <ArrowLeft size={16} />
            </Link>
            {receiver ? (
              <div className="flex items-center space-x-2.5">
                {receiver.profile_picture ? (
                  ...
                  <div className="w-8 h-8 rounded-full bg-indigo-950/50 border border-slate-800 flex items-center justify-center text-indigo-400 text-xs font-bold font-display">
                    {receiver.full_name.charAt(0)}
                  </div>
                )}
                <div className="space-y-0.5">
                  <h4 className="text-xs font-bold text-slate-200">{receiver.full_name}</h4>
                  <span className="text-[9px] text-slate-500 block truncate">{receiver.college} • {receiver.role}</span>
                </div>
              </div>
            ) : (
              <span className="text-xs font-semibold text-slate-400">Connecting peer session...</span>
            )}
          </div>
          
          <div className="text-[10px] text-indigo-400 bg-indigo-950/30 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
            Secure Channel
          </div>
        </div>
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-950/10">
          {messages.length === 0 ? (
            <div className="text-center py-20 text-slate-500 text-xs font-medium space-y-2">
              <MessageSquare size={24} className="mx-auto text-slate-700 animate-bounce" />
              ...
              return (
                <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-2xl p-3.5 space-y-1 ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-900 border border-slate-800 text-slate-200 rounded-tl-none'}`}>
                    ...
                    {/* Time & Read Receipts */}
                    <div className="flex items-center justify-end space-x-1 text-[9px] text-slate-400/80 pt-1">
                      <span>
        ...
        {/* Input box */}
        <form onSubmit={handleSendMessage} className="p-4 bg-slate-950/40 border-t border-slate-900 flex items-center space-x-3">
          ...
          <input 
            ...
            disabled={uploading || isLocked}
            className="flex-1 px-4 py-2.5 rounded-xl bg-slate-900/60 border border-slate-950 focus:border-indigo-500 focus:outline-none text-xs text-white disabled:opacity-50 disabled:cursor-not-allowed"
          />
