...
export default function PostTask() {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 space-y-8">
      {/* Page Header */}
      <div className="glass p-6 md:p-8 rounded-3xl border border-slate-800 space-y-1.5">
        <h2 className="text-2xl font-bold font-display text-white">Post Academic Task</h2>
        <p className="text-xs text-slate-400">Describe the study support requirements, allocate virtual budget, and leverage AI estimates.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        
        {/* Left Side: Forms */}
        <form onSubmit={handleSubmit} className="lg:col-span-2 glass p-6 md:p-8 rounded-3xl border border-slate-800 space-y-6">
          {error && (
          ...
          <div className="space-y-4">
            ...
            {/* Description */}
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5">Detailed Requirements & Context</label>
              <textarea 
                rows={6}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Completion Deadline</label>
                <input 
                  type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Budget Allocated (₹)</label>
                <input 
                  type="number"
                  placeholder="e.g. 1000"
                  value={budget}
                  onChange={(e) => setBudget(e.target.value)}
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-900/60 border border-slate-800 focus:border-indigo-500 focus:outline-none text-xs text-white font-semibold"
                  required
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Reference Documents</label>
                <div className="relative w-full">
                  ...
                  />
                  <div className="w-full px-4 py-2.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-400 flex items-center justify-between pointer-events-none">
          ...
          <button 
            ...
            className="w-full py-3.5 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center justify-center space-x-1.5 transition disabled:opacity-50"
          >
            <Send size={14} />
        ...
        <div className="glass p-6 rounded-3xl border border-slate-800 space-y-6">
          <h3 className="text-sm font-bold font-display flex items-center space-x-2 text-indigo-400 pb-2 border-b border-slate-900">
            <Brain size={16} />
          ...
          {scamAlert ? (
            ...
            title && description && (
              <div className="p-4 rounded-2xl bg-emerald-950/25 border border-emerald-800/40 flex items-start space-x-2 text-emerald-300 text-xs">
          ...
          {aiAnalyzing ? (
            ...
            <div className="space-y-4 pt-1">
              <div>
                <span className="text-[10px] text-slate-500 font-semibold uppercase block">Assessed Difficulty</span>
                <span className={`text-xs font-bold ${aiReport.difficulty === 'Hard' ? 'text-rose-400' : aiReport.difficulty === 'Medium' ? 'text-amber-400' : 'text-emerald-400'}`}>
                  {aiReport.difficulty} Level
                </span>
              </div>

              <div>
                <span className="text-[10px] text-slate-500 font-semibold uppercase block">Suggested Budget</span>
                <span className="text-xs font-bold text-slate-200">{aiReport.est_budget}</span>
              </div>

              <div>
                <span className="text-[10px] text-slate-500 font-semibold uppercase block">Estimated Effort</span>
                <span className="text-xs font-bold text-slate-200">{aiReport.est_time}</span>
              </div>

              <div className="text-[10px] text-slate-400 leading-relaxed pt-2 border-t border-slate-900 font-medium">
