...
async function seedPostgresData() {
  ...
  if (userCount === 0) {
    ...
    await pool.query(
      ...
      [MOCK_UUIDS.admin, 'admin@broplz.com', 'Admin Moderator', 'BroPlz HQ', 'Administration', 'Staff', 'Platform Administrator', 'admin', 5.0]
    );
...
function seedSqliteData() {
  ...
  if (userCount === 0) {
    ...
    sqliteDb.prepare(`
      INSERT INTO users (id, email, full_name, college, course, academic_year, bio, role, rating)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(MOCK_UUIDS.admin, 'admin@broplz.com', 'Admin Moderator', 'BroPlz HQ', 'Administration', 'Staff', 'Platform Administrator', 'admin', 5.0);
